"""
78. Subsets
https://leetcode.com/problems/subsets/description/

Given an integer array nums of unique elements, return all possible subsets
( the power set ).

The solution set must not contain duplicate subsets. Return the solution in any
order.

Example 1:

Input: nums = [1,2,3]
Output: [[],[1],[2],[1,2],[3],[1,3],[2,3],[1,2,3]]

Example 2:

Input: nums = [0]
Output: [[],[0]]

 

Constraints:

1 <= nums.length <= 10
-10 <= nums[i] <= 10
All the elements of nums are unique.
"""
from typing import List

class Solution:
    def subsets(self, nums: List[int]) -> List[List[int]]:
        result = []
        self.func(nums, 0, [], result)
        return result

    def func(self, nums: List[int], start: int, path: List[int], result: List[List[int]]):
        result.append(path.copy())  # Add the current subset to the result
        for i in range(start, len(nums)):
            path.append(nums[i])  # Include the current number in the subset
            self.func(nums, i + 1, path, result)  # Recur to create subsets with the next elements
            path.pop()  # Backtrack to explore other subsets

# Example of testing the function
if __name__ == "__main__":
    solution = Solution()
    nums = [1, 2, 3]
    subsets = solution.subsets(nums)
    print(subsets)  # Output: [[], [1], [1, 2], [1, 2, 3], [1, 3], [2], [2, 3], [3]]

"""
Time Complexity: O(2^n)
Space Complexity: O(n)
"""